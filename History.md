...
* **Changes**:
  - Fixed render gate condition in `CommonVillagerModelMixin` by keeping container `breasts.visible = true` so `renderCommon` renders `left_breast` and `right_breast` 3D model parts.
